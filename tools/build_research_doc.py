...
plan = next((r for r in results if isinstance(r, dict) and "changes" in r), None)
...
w("# Research Record — ADHD, music learning, and reinforcement")
w("")
...
for area in research:
    ...
    for f in area["findings"]:
        ...
        if f.get("appliesToAdultSelfDirectedPracticeApp"):
            w(f"- **Transfer to this app:** "
              f"{clean(f['appliesToAdultSelfDirectedPracticeApp'])}")
...
open("/home/user/fingerly/docs/RESEARCH.md", "w").write("\n".join(out) + "\n")
